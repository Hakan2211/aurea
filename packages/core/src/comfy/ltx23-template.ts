/* The native LTX 2.3 audio-video template, verbatim from the proven pipeline
 * (videofast/workflows/video_ltx2_3_ia2v.json — the exported built-in ComfyUI
 * "LTX 2.3 ia2v" workflow that solved lipsync on 2026-07-13). Every node is
 * core ComfyUI; video-graphs.ts patches inputs and (for the managed engine)
 * re-points the loaders, never the wiring. Embedded as TS so both tsx and the
 * electron bundler load it without JSON-module config. */

import type { ComfyGraph } from "./client.js";

export const LTX23_AV_TEMPLATE = {
  "269": {
    "inputs": {
  ...
  "276": {
    "inputs": {
      ...
      "audioUI": "/api/view?filename=ltx_23_audio.mp3&type=input&subfolder=&rand=0.7629285536387723"
    },
